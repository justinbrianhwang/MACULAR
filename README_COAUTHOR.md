...
#### 두 번째 백본 (Qwen2-VL-2B) — 순위가 바뀝니다

| 메커니즘 | 임상 F1 | 선형 | 비선형 | ctx선형 | ctx비선형 | ctxCER |
|---|---:|---:|---:|---:|---:|---:|
| none | 0.899 | 0.971 | 0.977 | 0.956 | 0.968 | 0.549 |
| hard_mask | 0.778 | 0.848 | 0.956 | 0.848 | 0.859 | **0.949** |
| gate | 0.826 | 0.848 | 0.967 | 0.923 | 0.929 | 0.743 |
| **leace** | **0.890** | **0.847** | **0.907** | 0.846 | 0.860 | 0.844 |

세 줄 요약:
1. **미분 가능 gate는 두 백본 모두에서 지배당합니다.** PaddleOCR-VL에서는 hard mask
   보다 효용도 낮고(0.917 vs 0.947) 누출도 크며, Qwen2-VL-2B에서는 LEACE에게 양쪽
   축에서 집니다(0.826 vs 0.890, ctx비선형 0.929 vs 0.860). **어떤 백본에서도,
   어떤 동작점에서도 gate가 정답인 경우가 없습니다.**
2. **LEACE vs hard mask는 백본에 따라 갈리고, 갈리는 이유가 측정됩니다.**
   Qwen에서는 LEACE 잔차 공분산이 0.480이라 선형 probe가 majority(0.847)까지
   떨어지고, hard mask와 **같은 ctx 누출(0.860 vs 0.859)에 효용 +0.112**를 냅니다.
   PaddleOCR-VL에서는 잔차가 0.771로 전이가 나빠서 hard mask가 이깁니다.
   즉 순위는 **백본이 PII를 얼마나 family-불변한 선형 구조로 인코딩하느냐**를
   따라갑니다 — 소거 기법의 성질이 아니라 특징 추출기의 성질입니다.
3. **원문 복원 방어는 두 백본 모두 hard mask가 최고입니다**(ctxCER 1.080 / 0.949 vs
   LEACE 0.549 / 0.844). 승자가 아니라 trade-off로 적어야 정확합니다.

전이 실패는 통제 실험으로 확정했습니다(`scripts/leace_transfer_control.py`):
val에 fit해서 val을 probe하면 선형 정확도가 **정확히 majority(0.847)**로 떨어집니다.
구현은 맞고 전이가 안 되는 겁니다. PaddleOCR-VL에서 저비용 수정(이진 개념·
shrinkage·rank 절단)과 다중 family 학습(A+D+E, 17,176 영역) 모두 실패했습니다
— 0.932 → 0.925.
