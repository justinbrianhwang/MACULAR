...
#### 백본 3종 비교 (graph 이후 = 실제 배포되는 표현, majority 0.847)

| 백본 | 메커니즘 | 임상 F1 | ctx선형 | ctx비선형 | ctxCER |
|---|---|---:|---:|---:|---:|
| PaddleOCR-VL | none | 0.976 | 0.958 | 0.975 | 0.454 |
...
**3종 모두에서 성립하는 것:**
1. **gate는 어디서도 지배당합니다.** 매 백본마다 더 단순한 방법이 양쪽 축에서 동시에
   이깁니다. Ministral에서는 보호 기법 중 가장 나쁩니다(ctx비선형 0.959 —
   보호 없는 0.975와 거의 차이가 없는데 효용은 깎입니다).
2. **LEACE의 보장은 어느 백본에서도 전이되지 않습니다** (교차 family 선형 probe
   0.889~0.944, majority 0.847). 저비용 수정도 다중 family 학습도 못 고칩니다.
3. **hard mask는 프라이버시에서 한 번도 지지 않습니다** — ctx비선형 항상 ≤ LEACE,
   원문 복원 방어(ctxCER)는 항상 최고.
4. **갈리는 건 효용 비용입니다.** hard mask 대비 LEACE의 효용 차이가
   −0.005(Paddle) / **+0.112**(Qwen) / **+0.166**(Ministral)로 크게 흔들립니다.

> **철회한 규칙:** 두 백본만 봤을 때 "LEACE 잔차 공분산이 순위를 예측한다"고
> 적었는데, Ministral이 반증했습니다(잔차는 셋 중 가장 낮은 0.463인데 교차 family
> probe는 가장 나쁜 0.944). 이유는 **잔차 공분산이 정규화되지 않은 특징에 대한
> Frobenius norm이라 백본 간 비교가 불가능**하기 때문입니다. 백본 간 비교가 되는
> 양은 probe 정확도입니다. 점 두 개로는 알아채지 못했습니다.
